...
async def category_trends(months: int = Query(default=4, ge=2, le=12), user_id: str = Depends(get_current_user), db: AsyncSession = Depends(get_session)):
    return await analytics_service.category_trends(user_id, months, db)
