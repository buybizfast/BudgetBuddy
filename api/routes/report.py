...
async def get_monthly_report(year: int, month: int, db: AsyncSession = Depends(get_session)):
    ...
    # Debts
    debts_result = await db.execute(select(DebtAccount).order_by(DebtAccount.sort_order))
    debts = [
    ...
    # Savings goals
    goals_result = await db.execute(select(SavingsGoal).order_by(SavingsGoal.sort_order))
    goals = [
...
async def export_csv(year: int, month: int, db: AsyncSession = Depends(get_session)):
    ...
    bm_result = await db.execute(
        select(BudgetMonth)
        .where(BudgetMonth.year == year, BudgetMonth.month == month)
