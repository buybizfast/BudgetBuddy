...
export function useBudget(year: number, month: number) {
  ...
  const updateCategoryDueDate = async (id: string, day: number | null) => {
    await apiFetch(`/api/v1/budget/categories/${id}`, { method: 'PATCH', body: JSON.stringify({ due_date_day: day ?? 0 }) })
    await refresh()
  }
  ...
  const reorderCategories = async (groupId: string, categoryIds: string[]) => {
    setBudget(prev => {
      if (!prev) return prev
      return {
        ...prev,
        groups: prev.groups.map(g => g.id !== groupId ? g : {
          ...
          categories: categoryIds
            .map(id => g.categories.find(c => c.id === id))
            .filter((c): c is BudgetCategory => !!c),
        }),
      }
    })
    await apiFetch(`/api/v1/budget/groups/${groupId}/categories/reorder`, {
      method: 'PATCH',
